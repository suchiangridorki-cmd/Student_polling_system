...
$user_query = mysqli_query($conn, "SELECT name, role FROM users WHERE user_id='$user_id'");
...
$poll_results = mysqli_query($conn, "SELECT p.poll_id, p.title, o.option_id, o.option_text, COUNT(v.vote_id) as votes
                                     FROM polls p
...
$vote_query = mysqli_query($conn, "
    SELECT p.poll_id, o.option_text 
    FROM votes v
...
while($v = mysqli_fetch_assoc($vote_query)){
    $user_votes[$v['poll_id']] = $v['option_text'];
}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; 
        ...
        .sidebar {
            width: 250px; background: #9fcfe9; 
            height: 100vh; 
            padding: 20px; 
            position: fixed; 
            transition: 0.3s;
        }
        .sidebar h2 { color:#21465c; 
        margin-bottom:10px; 
    }
        ...
        .sidebar a:hover {background: #d1e4f3; 
    }
        .logout { background:#efbdc2;; 
    }
    ...
    .logo{
        ...
        .recent-polls h2 { margin-bottom:15px; 
    }
        .poll-card {
            background:#4f8bbd;
            color:#fff; 
        ...
        .poll-card:hover { background: rgba(77, 109, 122, 0.2); 
        ...
        .poll-results h2 { color:#21465c; 
        margin-bottom:15px; 
    }
        .poll-results .poll-card span { color:#21465c; 
        margin-left:10px; 
    }
        .poll-results .poll-card strong { color:#21465c; 
    }
...
<div class="layout">
    ...
    <div class="main">
        ...
        <div class="poll-results">
            ...
            <?php
            $current_poll = 0;
            mysqli_data_seek($poll_results,0); 
            while($row = mysqli_fetch_assoc($poll_results)){
                if($current_poll != $row['poll_id']){
                    if($current_poll != 0) echo "</div>";
                    echo "<div class='poll-card' style='flex-direction:column; align-items:flex-start; margin-top:15px;'>";
                    echo "<strong>Poll:</strong> " . $row['title'];

if(isset($user_votes[$row['poll_id']])){
    echo " <span style='color:black; font-size:13px;'>(You voted: ".$user_votes[$row['poll_id']].")</span>";
}

echo "<br>";
                    $current_poll = $row['poll_id'];
                }
                echo "<span>• " . $row['option_text'] . " - <strong>" . $row['votes'] . "</strong> votes</span><br>";
            }
            if($current_poll != 0) echo "</div>";
            ?>
        </div>
